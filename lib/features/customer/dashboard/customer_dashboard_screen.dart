...
final customerDashboardProvider = FutureProvider((ref) async {
  ...
  final transactions = await supabaseService.getTransactionsForCustomer(profile.id);
...
class CustomerDashboardScreen extends ConsumerWidget {
  ...
  void _showSubmitComplaintDialog(BuildContext context, WidgetRef ref, CustomerModel profile) {
            ...
            actions: [
              ...
              TextButton(
                onPressed: isLoading ? null : () async {
                  ...
                  try {
                    await ref.read(supabaseServiceProvider).submitComplaint(
                      profile.id,
                      profile.ownerId,
                    ...
                    Navigator.pop(ctx);
                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Message submitted successfully')));
                  } catch (e) {
                    if (!context.mounted) return;
              ...
              builder: (context, ref, child) {
                ...
                return dashboardAsync.when(
                  data: (data) {
                    ...
                    final statusText = FinancialCalculator.getStatusText(status);

                    Color statusColor = Colors.white;
                    if (status == PaymentStatus.overdue) statusColor = Colors.redAccent;
                    if (status == PaymentStatus.paid) statusColor = Colors.greenAccent;
              ...
              builder: (context, ref, child) {
                ...
                return Card(
                  ...
                  child: ListTile(
                    ...
                    title: const Text('Submit Complaint / Request'),
                    trailing: const Icon(Icons.chevron_right),
                    onTap: profile == null ? null : () => _showSubmitComplaintDialog(context, ref, profile),
                  ),
                );
              }
            ),
